...
RETENTION = BUCKET * 2 * 1000  # retention (ms)
...
def inputData(msg, r):
    ...
    unixTime = int(time.mktime(
        datetime.datetime.strptime(day, "%Y-%m-%d").timetuple()))  # in seconds
    ...
    for commandIndex in range(2, len(commands)):
        ...
        try:
            ...
            print(newKeyAll, newKey, "avg", BUCKET)
...
if __name__ == '__main__':
    ...
    redis_thread = threading.Thread(
        target=restartRedis, args=("/home/antho/redisTemp.conf",))
    redis_thread.start()
    time.sleep(3)
